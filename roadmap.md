...
> Estado actual: Fases 0-3 completadas. Fase 4 (integración hogarOS) casi completa, solo falta la tarjeta resumen en el bento grid. Catálogo con CRUD completo, estado `por_capturar` para encolar enriquecimiento, campo `ean`, y formulario en **ventana modal** (antes empujaba la página, commit `3d65406`). Pestaña Tickets funcional (commit `6f8adf8`), pendiente de desplegar. **Rutina de Cowork para capturar productos `por_capturar`** (`rutinas/prompt_capturar_producto.md`) creada y validada con una ejecución manual real: API pública de Mercadona, desambigua por precio, `wh=svq1` fijado, guarda `ean`. Sigue en pruebas manuales, sin temporización. **Rediseño del portal en marcha**: Catálogo agrupado por marca (commit `00426fb`) y **Despensa agrupada por zona** (commit `5bd2ef0`) — de paso se arregló un bug real: `rutas/stock.py` nunca traía `zona`/`stock_minimo`/`unidad` en el JOIN con catálogo, así que el filtro "Stock bajo/OK" llevaba roto en silencio desde siempre. Decisión de diseño confirmada con el usuario: `zona` vive en `catalogo` (habitual del producto, no del lote de stock) a propósito — el stock la hereda vía `producto_id`, no tiene `zona` propia. Detalle línea a línea en `bitacora.md`, entradas "2026-08-02", "2026-08-03" (dos) y "2026-08-04".
> Última actualización: 2026-08-04
> **Próximo paso concreto:**
> 1. 👤 Ejecutar `actualizar.sh` en la VM para desplegar todo lo pendiente: campo `ean` y catálogo agrupado (`148a0ba`, `00426fb`), modal de catálogo (`3d65406`), Despensa agrupada por zona + fix del JOIN (`5bd2ef0`), y la pestaña Tickets (`6f8adf8`)
> 2. 👤 Verificar visualmente en producción: modal de catálogo con EAN, catálogo agrupado por marca, Despensa agrupada por zona con el filtro Stock bajo/OK ya funcionando, y ticket #5 en la pestaña Tickets (25 líneas correctas al expandir)
...
> 5. 🤖/👤 Seguir con el rediseño del resto de pestañas del portal (Lista, Caducidades, Tickets) si se quiere aplicar el mismo criterio
